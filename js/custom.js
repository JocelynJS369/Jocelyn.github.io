document.addEventListener('DOMContentLoaded', function() {
  ...
  const carousel = {
    ...
    indicators: document.querySelector('.gallery-indicators'),
    currentIndex: 0,
    interval: null,
    ...
    init: function() {
      ...
      // 添加按钮事件
      document.querySelector('.gallery-prev').addEventListener('click', () => this.prevSlide());
      document.querySelector('.gallery-next').addEventListener('click', () => this.nextSlide());
      
      // 添加指示器事件
      this.indicators.querySelectorAll('span').forEach(indicator => {
    ...
    showSlide: function(index) {
      ...
      // 更新指示器
      this.indicators.querySelectorAll('span').forEach((indicator, i) => {
